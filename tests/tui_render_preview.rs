...
fn preview() {
    let metrics = Metrics::shared();
    {
        ...
        for i in 0..3 {
            m.begin(format!("What is the capital of country #{i} and its largest city? Provide details."), 61, "d4e5f6".into());
            m.tick(28 + i);
            m.finish(212.0, 912.0, 18, 42, 27);
        }
        m.begin("Explain the difference between stable merge sort and in-place.".to_string(), 74, "d4e5f6".into());
        m.tick(15);
    }
