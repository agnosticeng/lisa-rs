...
pub fn run(args: &[String]) -> Result<(), String> {
    ...
    if speculative.tokens != target_tokens {
        return Err(format!(
            "speculative output differs from target-only at token {}",
            speculative
                .tokens
                .iter()
                .zip(&target_tokens)
