...
fn draw(
    ...
    let area = frame.area();
    let [header_area, _, stats_area, _, table_area, footer_area] = Layout::vertical([
        Constraint::Length(1),
        Constraint::Length(1),
        Constraint::Length(3),
        Constraint::Length(1),
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::styled(format!("{prefill_label}"), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
            Span::styled(" tok/s", Style::default().fg(Color::DarkGray)),
            Span::raw("  "),
            Span::styled("Decode ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{decode_label}"), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
            Span::styled(" tok/s", Style::default().fg(Color::DarkGray)),
            Span::raw("  |  "),
            ...
            Span::raw("  |  "),
            Span::styled(format!("{total_tok} tok"), Style::default().fg(Color::White)),
            Span::raw(" "),
            Span::styled(format!("{avg_tok_s:.1} avg"), Style::default().fg(Color::DarkGray)),
        ]))
...
fn query_table(queries: &[QueryRecord], scroll: usize) -> Table<'static> {
    let header = Row::new(vec![
        "#", "TIME", "SESSION", "PROMPT", "PT", "CT", "PREF ms", "PREF t/s", "DEC t/s", "ACC%", "DRFT", "FWD",
    ])
    .style(
        Style::default()
            .fg(Color::White)
    ...
    let rows: Vec<Row> = queries
        ...
        .map(|(_, q)| {
            ...
            let session_style = session_style(&q.session);
            Row::new(vec![
                format!("#{}", q.seq),
                time,
                format!("{}", q.session),
...
/// Deterministic per-session color so rows of one conversation share a tint.
fn session_style(session: &str) -> Style {
    ...
    let fg = palette[v as usize % palette.len()];
    Style::default().fg(fg).add_modifier(if fg == Color::White {
        Modifier::BOLD
    } else {
        Modifier::empty()
    })
...
mod tests {
    ...
    fn renders_populated_dashboard() {
        let metrics = Metrics::shared();
        {
            ...
            for i in 0..3 {
                m.begin(format!("What is the capital of country #{i}?"), 55, "a1b2c3".into());
                m.tick(20 + i);
                m.finish(350.0, 900.0, 18, 40, 25);
            }
            m.begin("And also this longer question in flight...".to_string(), 80, "a1b2c3".into());
            m.tick(10);
        }
        render(&metrics, "qwen3.8-27b");
    }
    ...
    fn metrics_aggregate_correctly() {
        let metrics = Metrics::shared();
        let mut m = metrics.lock().unwrap();
        m.begin("prompt a".to_string(), 10, "aaaaaa".into());
        m.tick(5);
        m.finish(100.0, 500.0, 4, 8, 6);
        m.begin("prompt b".to_string(), 20, "bbbbbb".into());
        m.tick(3);
    ...
    fn reset_clears_metrics() {
        ...
        {
            let mut m = metrics.lock().unwrap();
            m.begin("x".to_string(), 5, "cccccc".into());
            m.tick(2);
