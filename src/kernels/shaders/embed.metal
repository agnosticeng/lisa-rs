...
// Batched variant of argmax_f32_partial: one dispatch over grid (n/256, m)
// reduces every logits row of a prefill chunk in a single pass. Row r writes
// its partials at `r*stride` in the SAME layout as the per-row kernel
// (maxes then idx), so the host readback code is shared. Dispatches per chunk:
// 1 instead of `m` (m=512), removing hundreds of tiny serial dispatches.
kernel void argmax_f32_rows(
    device const float* x [[buffer(0)]],
    device float* partial_max [[buffer(1)]],
    device uint* partial_idx [[buffer(2)]],
    constant uint& n [[buffer(3)]],
  ...
  threadgroup uint smi[8];
  const uint row = tg.y;
  const uint i = tg.x * 256u + sg * 32u + lane;
  float v = (i < n) ? x[row * n + i] : -FLT_MAX;
  uint idx = i;
  for (uint s = 16u; s > 0u; s >>= 1u) {
    const float other = simd_shuffle_down(v, s);
    const uint oi = simd_shuffle_down(idx, s);
  ...
  if (sg == 0u && lane == 0u) {
    ...
    partial_max[row * stride + tg.x] = best;
    partial_idx[row * stride + tg.x] = bi;
  }
}
