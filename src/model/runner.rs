...
/// Number of KV-token slices used by the flash-style sliced decode attention
/// (sdpa_decode_partial). The decode context is partitioned into this many
/// disjoint ranges, each run by a separate threadgroup, then merged by
/// sdpa_decode_final. Buffers are sized for the maximum slice count; the actual
/// dispatch enables only enough slices to cover the current context.
const ATTN_SLICES: usize = 256;
/// Each slice scans up to this many KV tokens before the merge. Slice count for
/// one decode = min(ATTN_SLICES, ceil(context / TOKENS_PER_SLICE)).
const TOKENS_PER_SLICE: usize = 64;
/// A/B switch: use the flash-style sliced decode attention (true) or the
/// original serial sdpa_decode_streaming[_block3] (false). Keep until the
/// near-tie argmax drift is re-characterized vs baseline.
const SLICED_ATTN: bool = true;
...
struct Pipelines {
    ...
    q4_gemm_align: ComputePipeline,
    q4_gemm_align_fused2: ComputePipeline,
    ...
    attn_sdpa_partial: ComputePipeline,
    attn_sdpa_final: ComputePipeline,
...
impl ScratchLayout {
    fn new(batch: usize, attn_slices: usize) -> Self {
        let mut next = 0usize;
        ...
        let attn_partial = take(batch * ATTN_HEADS * attn_slices * ATTN_HEAD_DIM * 4);
        let attn_stat = take(batch * ATTN_HEADS * attn_slices * 2 * 4);
...
impl QwenRunner {
    pub fn load(snapshot: &Path, capacity: usize) -> Result<Self, String> {
        ...
        let scratch_layout = ScratchLayout::new(3, ATTN_SLICES);
        let scratch = device.new_buffer(scratch_layout.len);
        let prefill_layout = ScratchLayout::new(PREFILL_BATCH, 0);
    ...
    /// The shared target scratch buffer (for device-resident hidden carry).
    pub fn scratch(&self) -> &MetalBuffer {
        &self.scratch
    }

    /// Byte offset in `scratch` of a batch-3 verify's row `row` hidden (bf16,
    /// HIDDEN elements). Only valid right after `verify_block3_decode*`.
    pub fn hidden_offset(&self, row: usize) -> usize {
        self.scratch_layout.hidden + row * HIDDEN * 2
    }

    /// Upload a host `[f32; HIDDEN]` hidden into the scratch at
    /// `hidden_offset(row)` as bf16 (used once to seed the first device draft).
    pub fn upload_hidden_device(&self, row: usize, hidden: &[f32]) {
        let bytes: Vec<u8> = hidden
            .iter()
            .flat_map(|value| f32_to_bf16(*value).to_le_bytes())
            .collect();
        self.scratch.write_bytes(self.hidden_offset(row) as u64, &bytes);
    }

    /// Device-resident variant of `verify_block3_decode`: identical fused batch-3
    /// forward, but reads back ONLY the three GPU argmaxes. The per-row hidden
    /// states (bf16) are left in `self.scratch` at `hidden_offset(row)` so the
    /// next MTP draft can consume them without a host round trip. This removes
    /// the (bf16->f32 readback, f32->bf16 re-upload) that the host loop did
    /// between a verify and the following MTP forward — a per-round sync.
    pub fn verify_block3_decode_device(&mut self, tokens: [u32; 3]) -> Result<[u32; 3], String> {
        if tokens.iter().any(|token| *token as usize >= VOCAB) {
            return Err("verify token is outside vocabulary".into());
        }
        if self.position + 3 > self.capacity {
        ...
        let a0 = self.read_argmax(0);
        let a1 = self.read_argmax(1);
        let a2 = self.read_argmax(2);
        self.position += 3;
        self.verify_base_position = Some(base);
        Ok([a0, a1, a2])
    }
    ...
    fn encode_attention(&self, encoder: &ComputeEncoder, weights: &AttentionWeights, batch: usize) {
        ...
        if SLICED_ATTN {
            // Flash-style sliced decode attention: stage per-slice partial
            // accumulators + stats, then a tiny merge kernel reduces them into
            // the normalized attention output. Parallelizes the KV-scan
            // bottleneck at long context. Partition is a pure function of each
            // row's context (fixed TOKENS_PER_SLICE chunks), so M=1 and batch-3
            // verify are bit-identical per row.
            let grid_slices = (((self.position + batch) / TOKENS_PER_SLICE)
                + if (self.position + batch) % TOKENS_PER_SLICE == 0 { 0 } else { 1 })
                .min(ATTN_SLICES);
            encoder.set_compute_pipeline_state(&self.pipelines.attn_sdpa_partial);
            ...
            encoder.set_buffer(&self.scratch, s.attn_partial as u64, 3);
            encoder.set_buffer(&self.scratch, s.attn_stat as u64, 4);
            encoder.set_bytes(&(self.position as u32).to_le_bytes(), 5);
            encoder.set_bytes(&(self.capacity as u32).to_le_bytes(), 6);
            encoder.set_bytes(&(batch as u32).to_le_bytes(), 7);
            encoder.dispatch_thread_groups(
                MTLSize {
                    width: 24,
                    height: batch as u64,
                    depth: grid_slices as u64,
                },
                MTLSize {
                    width: 32,
                    height: 1,
            ...
            encoder.set_compute_pipeline_state(&self.pipelines.attn_sdpa_final);
            encoder.set_buffer(&self.scratch, s.attn_partial as u64, 0);
            encoder.set_buffer(&self.scratch, s.attn_stat as u64, 1);
            encoder.set_buffer(&self.scratch, s.attention_output as u64, 2);
            encoder.set_bytes(&(self.position as u32).to_le_bytes(), 3);
            encoder.dispatch_thread_groups(
                MTLSize {
                    width: 24,
                    height: batch as u64,
            ...
            // Original serial streaming decode (A/B baseline).
            encoder.set_compute_pipeline_state(if batch == 3 {
            ...
            // Serial streaming SDPA: one 32-lane threadgroup per (head, row).
            encoder.dispatch_thread_groups(
                MTLSize {
                    width: 24,
                    height: batch as u64,
    ...
    fn encode_gemm(
        ...
        // Branchless 64-aligned GEMM for the common projections (M,N %64==0);
        // mask-predicated tiled fallback for the small unaligned GDN gates.
        let aligned = linear.output % 64 == 0;
        if aligned {
            encoder.set_compute_pipeline_state(&self.pipelines.q4_gemm_align);
        } else {
        ...
        // Tiled cooperative GEMM: 64x64 output tile per threadgroup (128
        // threads). Grid x spans M in 64-row tiles, grid y spans N in 64-col
        // tiles. M/N are padded up to tile multiples by the host (M rows are
        // masked in-store; N cols are bounds-checked in the dequant).
        let g = m.div_ceil(64);
        let m64 = g * 64;
        let mut mnk = Vec::with_capacity(12);
        mnk.extend_from_slice(&(m64 as u32).to_le_bytes());
        mnk.extend_from_slice(&(linear.output as u32).to_le_bytes());
        mnk.extend_from_slice(&(linear.input as u32).to_le_bytes());
        encoder.set_bytes(&mnk, 5);
        encoder.dispatch_thread_groups(
            MTLSize {
                width: g as u64,
                height: linear.output.div_ceil(64) as u64,
                depth: 1,
            },
            MTLSize {
                width: 128,
    ...
    // Fused two-projection tiled align GEMM (gate/up in one dispatch). Both
    // linears must share M,K; grid.z selects which weight/output pair runs.
    fn encode_gemm_fused2(
        &self,
        encoder: &ComputeEncoder,
        scratch: &MetalBuffer,
        m: usize,
        ...
        debug_assert_eq!(a.output, b.output);
        encoder.set_compute_pipeline_state(&self.pipelines.q4_gemm_align_fused2);
        encoder.set_buffer(scratch, input as u64, 0);
        encoder.set_buffer(self.weight_buffer(a.weight), a.weight.offset, 1);
        encoder.set_buffer(self.weight_buffer(a.scales), a.scales.offset, 2);
        encoder.set_buffer(self.weight_buffer(a.biases), a.biases.offset, 3);
        encoder.set_buffer(scratch, output_a as u64, 4);
        encoder.set_buffer(self.weight_buffer(b.weight), b.weight.offset, 5);
        encoder.set_buffer(self.weight_buffer(b.scales), b.scales.offset, 6);
        encoder.set_buffer(self.weight_buffer(b.biases), b.biases.offset, 7);
        encoder.set_buffer(scratch, output_b as u64, 8);
        let g = m.div_ceil(64);
        let m64 = g * 64;
        let mut mnk = Vec::with_capacity(12);
        mnk.extend_from_slice(&(m64 as u32).to_le_bytes());
        mnk.extend_from_slice(&(a.output as u32).to_le_bytes());
        mnk.extend_from_slice(&(a.input as u32).to_le_bytes());
        encoder.set_bytes(&mnk, 9);
        encoder.dispatch_thread_groups(
            MTLSize {
                width: g as u64,
                height: a.output.div_ceil(64) as u64,
                depth: 2,
            },
            MTLSize {
                width: 128,
    ...
    fn encode_prefill_mlp(
        ...
        self.encode_gemm_fused2(encoder, scratch, batch, post_norm, &weights.gate, gate, &weights.up, up);
    ...
    fn encode_prefill_argmax(&self, encoder: &ComputeEncoder, scratch: &MetalBuffer, m: usize) {
        const TG: usize = 256;
        let partials = VOCAB.div_ceil(TG);
        let stride = partials * 2;
        let input = self.prefill_layout.logits;
        let base = self.prefill_layout.argmax_partial;
        encoder.set_compute_pipeline_state(&self.pipelines.argmax_rows);
        encoder.set_buffer(scratch, input as u64, 0);
        encoder.set_buffer(scratch, base as u64, 1);
        encoder.set_buffer(scratch, (base + partials * 4) as u64, 2);
        encoder.set_bytes(&(VOCAB as u32).to_le_bytes(), 3);
        encoder.set_bytes(&(stride as u32).to_le_bytes(), 4);
        encoder.dispatch_thread_groups(
            MTLSize {
                width: partials as u64,
                height: m as u64,
    ...
    /// Mirrors `forward_prefill_with_progress` without the progress callback.
    pub fn forward_prefill(&mut self, tokens: &[u32]) -> Result<Vec<(u32, Vec<f32>)>, String> {
        self.forward_prefill_with_progress(tokens, |_| {})
    }
    ...
    /// `on_chunk(processed)` fires after each fused chunk with the number of
    /// tokens fully prefilled so far (used to stream live prefill speed).
    pub fn forward_prefill_with_progress(
        &mut self,
        tokens: &[u32],
        ...
        while base < tokens.len() {
            ...
            self.encode_prefill_argmax(&encoder, &self.prefill_scratch, m);
...
pub struct MtpRunner {    _device: MetalDevice,
    _index: WeightIndex,
...
/// GPU-resident snapshot of the MTP runner's KV caches + logical position,
/// used for session-prefix prefill reuse across requests.
...
impl MtpRunner {
    /// Capture the MTP KV caches (device-to-device copy) + logical position.
    pub fn checkpoint_state(&self) -> MtpCheckpoint {
        let cache_len = ATTN_KV_HEADS * self.capacity * ATTN_HEAD_DIM * 2;
        let key = self._device.new_buffer(cache_len);
        let value = self._device.new_buffer(cache_len);
        let command = self.queue.new_command_buffer();
        let encoder = command.compute_compute_encoder();
        self.copy_cache(&encoder, &self.key_cache, &key, cache_len);
        self.copy_cache(&encoder, &self.value_cache, &value, cache_len);
        encoder.end_encoding();
        command.commit();
        command.wait_until_completed();
        MtpCheckpoint {
    ...
    /// Restore the MTP KV caches + position from a checkpoint.
    pub fn restore_state(&mut self, checkpoint: &MtpCheckpoint) -> Result<(), String> {
        if checkpoint.position > self.capacity {
            return Err(format!(
                "MTP checkpoint position {} exceeds capacity {}",
                checkpoint.position, self.capacity
            ));
        }
        let cache_len = ATTN_KV_HEADS * self.capacity * ATTN_HEAD_DIM * 2;
        let command = self.queue.new_command_buffer();
        let encoder = command.compute_compute_encoder();
        self.copy_cache(&encoder, &checkpoint.key_cache, &self.key_cache, cache_len);
        self.copy_cache(&encoder, &checkpoint.value_cache, &self.value_cache, cache_len);
        encoder.end_encoding();
        command.commit();
        command.wait_until_completed();
        self.position = checkpoint.position;
    ...
    fn copy_cache(&self, encoder: &ComputeEncoder, src: &MetalBuffer, dst: &MetalBuffer, len: usize) {
        const ROW: u64 = 1 << 30;
        let chunks = len.div_ceil(16) as u64;
        encoder.set_compute_pipeline_state(&self.pipelines.copy_bytes);
    ...
    pub fn load(target: &QwenRunner, snapshot: &Path, capacity: usize) -> Result<Self, String> {
        ...
        let scratch_layout = MtpScratchLayout::new(1, ATTN_SLICES);
        let cache_bytes = ATTN_KV_HEADS
            .checked_mul(capacity)
            .and_then(|value| value.checked_mul(ATTN_HEAD_DIM * 2))
            .ok_or_else(|| "MTP KV capacity overflow".to_owned())?;
        let scratch = device.new_buffer(scratch_layout.len);
        let prefill_layout = MtpScratchLayout::new(PREFILL_BATCH, 0);
        let prefill_scratch = device.new_buffer(prefill_layout.len);
        let key_cache = device.new_buffer(cache_bytes);
        let value_cache = device.new_buffer(cache_bytes);
        let queue = device.new_command_queue();
    ...
    /// Set the logical position directly (for session-prefix cache reuse). The
    /// KV rows already in the buffers are preserved; any rows beyond the new
    /// position are treated as stale and overwritten on the next forward.
    pub fn set_position(&mut self, position: usize) -> Result<(), String> {
        if position > self.position {
            return Err(format!(
                "cannot extend MTP position from {} to {position} without a forward",
                self.position
            ));
        }
    ...
    /// The shared MTP scratch buffer (for device-resident hidden carry).
    pub fn scratch(&self) -> &MetalBuffer {
        &self.scratch
    }

    /// Byte offset in `scratch` of this runner's own final normalized hidden
    /// (bf16) — the conditioning state the next MTP draft consumes. Valid right
    /// after `forward_position_decode*`.
    pub fn normalized_offset(&self) -> usize {
        self.scratch_layout.normalized
    }

    /// Upload a host `[f32; HIDDEN]` into `scratch` at `normalized_offset` as
    /// bf16 (used once to seed the first device-resident draft).
    pub fn upload_normalized_device(&self, hidden: &[f32]) {
        let bytes: Vec<u8> = hidden
            .iter()
            .flat_map(|value| f32_to_bf16(*value).to_le_bytes())
            .collect();
        self.scratch
            .write_bytes(self.normalized_offset() as u64, &bytes);
    }

    /// Device-resident draft forward: reads the conditioning hidden (bf16) from
    /// an explicit source `buffer`/`offset` (either this runner's own
    /// `normalized_offset` for draft chaining, or the target runner's
    /// `hidden_offset` for the replacement), and leaves its own final
    /// normalized hidden (bf16) on-device, returning its offset. Reads back only
    /// the lm_head argmax, so the speculative loop carries both target and MTP
    /// hidden entirely on-device (one host sync per call instead of two).
    pub fn forward_position_decode_device(
        &mut self,
        token: u32,
        cond: Option<&MetalBuffer>,
        cond_off: usize,
        ...
        // `cond`: None means condition on this runner's own scratch (draft
        // chaining); Some(src) means condition on another runner's hidden
        // (the target's verified hidden for the replacement).
        let cond_buf = cond.unwrap_or(&self.scratch);
        self.encode_norm_from(
            &encoder,
            cond_buf,
            cond_off,
            self.weights.pre_fc_norm_hidden,
    ...
    /// RMSNorm that reads its input from an explicit source `buffer`/`offset`
    /// (the target runner's scratch when carrying hidden on-device) instead of
    /// `self.scratch`. Same numerics as `encode_norm`.
    fn encode_norm_from(
        &self,
        encoder: &ComputeEncoder,
        src: &MetalBuffer,
        ...
        encoder.set_compute_pipeline_state(&self.pipelines.rms);
        encoder.set_buffer(src, input as u64, 0);
        encoder.set_buffer(self.mtp_weight_buffer(weight), weight.offset, 1);
        encoder.set_buffer(&self.scratch, output as u64, 2);
        encoder.set_bytes(&(HIDDEN as u32).to_le_bytes(), 3);
        encoder.set_bytes(&EPS.to_le_bytes(), 4);
    ...
    fn encode_attention(&self, encoder: &ComputeEncoder) {
        ...
        if SLICED_ATTN {
            ...
            encoder.set_bytes(&(self.position as u32).to_le_bytes(), 5);
            encoder.set_bytes(&(self.capacity as u32).to_le_bytes(), 6);
            let grid_slices = ((self.position + 1) / TOKENS_PER_SLICE
                + if (self.position + 1) % TOKENS_PER_SLICE == 0 { 0 } else { 1 })
                .min(ATTN_SLICES);
    ...
    /// Batch-prefill `positions` on the MTP drafter, mirroring `forward_prefill`
    /// (GEMM projections + batched causal attention) in fused chunks.
    pub fn forward_position_batch(
        &mut self,
        positions: &[(u32, &[f32])],
    ) -> Result<Vec<(u32, Vec<f32>)>, String> {
        self.forward_position_batch_with_progress(positions, |_| {})
    }

    /// `forward_position_batch` plus a per-chunk progress callback.
    pub fn forward_position_batch_with_progress(
        &mut self,
        positions: &[(u32, &[f32])],
        mut on_chunk: impl FnMut(usize),
    ) -> Result<Vec<(u32, Vec<f32>)>, String> {
        ...
        while base < positions.len() {
            ...
            self.mtp_prefill_argmax(&encoder, m);
            encoder.end_encoding();
            command.commit();
            command.wait_until_completed();
    ...
    fn mtp_prefill_argmax(&self, encoder: &ComputeEncoder, m: usize) {
        const TG: usize = 256;
        let partials = VOCAB.div_ceil(TG);
        let stride = partials * 2;
        let input = self.prefill_layout.logits;
...
impl Pipelines {
    fn new(device: &MetalDevice) -> Self {
        ...
        Self {
            ...
            q4_gemm: device.new_compute_pipeline(&q4.function_named("q4_gemm_nax_tiled")),
            q4_gemm_align: device.new_compute_pipeline(&q4.function_named("q4_gemm_nax_tiled_align")),
            q4_gemm_align_fused2: device.new_compute_pipeline(&q4.function_named("q4_gemm_nax_tiled_align_fused2")),
            ...
            attn_sdpa_partial: device
                .new_compute_pipeline(&attention.function_named("sdpa_decode_partial")),
            attn_sdpa_final: device
                .new_compute_pipeline(&attention.function_named("sdpa_decode_final")),
            ...
            argmax_rows: device.new_compute_pipeline(&embed.function_named("argmax_f32_rows")),
        }
    }
}
